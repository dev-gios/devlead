...
**Resolución del título y política por-issue (una sola llamada):** para cada `#N` declarado, resolvé la línea con `gh issue view {N} --json title,labels,body` — UNA sola llamada que devuelve los tres campos en un solo payload JSON (widening de la llamada anterior `--json title -q .title`; NO agregás round-trips). Consumo de los tres campos:
- **title** → la línea de cola, igual que hoy (`1. #[N] — [título]`). Extraelo con `-q .title` o `jq -r .title` del mismo payload.
- **labels** → array de objetos `[{"name":"wip",...}]`; extraé los nombres con `-q '.labels[].name'` o `jq -r '.labels[].name'` (NO la forma tab/coma-joined de `state.sh` — acá leés directo de `gh`, así que obtenés el array JSON nativo). Alimenta el chequeo de exclude-label.
- **body** → texto crudo de la issue; extraelo con `-q .body` o `jq -r '.body // ""'`. Alimenta el chequeo de readiness, trimmeando con `sed 's/^[[:space:]]*//;s/[[:space:]]*$//'` (misma expresión que envelope.sh:630).
...
- **Chequeo de readiness** (espejo exacto de envelope.sh:624-635, gated en `REQUIRE_READINESS`): SOLO si `REQUIRE_READINESS` de `show` es `true` (mismo guard que envelope.sh:625). Trimmeá el body con `sed 's/^[[:space:]]*//;s/[[:space:]]*$//'` (envelope.sh:630); si el body trimmeado queda vacío, renderizá:
...
- La frase "procesando igual por autorización explícita" es LOAD-BEARING: refuerza REQ-2 inline (informativo, no un gate) — el usuario nunca debe leer la advertencia como que algo se bloqueó. La issue SIEMPRE se procesa, sin importar el resultado del chequeo.
