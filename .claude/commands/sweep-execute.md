...
**Nota sobre las anotaciones de E0:** las etiquetas `cannot-cd`, `skipped`, `auth-unavailable` usadas arriba son labels de PREVIEW SOLAMENTE — son previsualizaciones de los STATUS canónicos de E1.x (p.ej. `skipped` aquí corresponde a `STATUS: skipped — disabled (ENABLED: false) — the envelope's kill-switch is off by choice. No action needed.` o `STATUS: skipped — no envelope.yml (ENROLLED: false) — listed in the fleet but nothing authorizes work here. Fix: cd '$repo_path' && devlead init` en E1.2). El STATUS canónico y autoritativo se emite durante el outer loop (E1.1–E1.4), no en E0.
...
Si `ENABLED: false`:
- Registrá `REPO → STATUS: skipped — disabled (ENABLED: false) — the envelope's kill-switch is off by choice. No action needed.`
- Continuá con el siguiente repo.
...
**STATUS: skipped — disabled (ENABLED: false) — the envelope's kill-switch is off by choice. No action needed.**
...
| `STATUS: skipped — disabled (ENABLED: false) — the envelope's kill-switch is off by choice. No action needed.` | ENABLED: false en E1.2 |
